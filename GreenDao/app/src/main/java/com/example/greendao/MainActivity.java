...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mGreenDaoAdapter.setOnItemLongClickListener(new GreenDaoAdapter.OnItemLongClickListener() {
            ...
            public void onLongClick(View view, User item) {
                if (userList.size() != 0 ){
                    GreenDaoHelper.deleteSingle(MainActivity.this,item);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btn:
                GreenDaoHelper.deleteAll(this);
                userList.clear();
                mGreenDaoAdapter.resetData(userList);
                break;
            case R.id.btn_search:
                userList = GreenDaoHelper.queryAll(this);
    ...
    private void insertDB(String name) {
        ...
        user.setName(name);
        userList.add(user);
        GreenDaoHelper.insertSingle(this,user);
    }
}